...
final class ModuleService
{
    ...
    public function list(string $entity, array $filters = [], int $page = 1, int $size = 25): array
    {
        ...
        $queries = [
            ...
            'hakedisler' => "SELECT h.*,s.ad sozlesme_adi,i.ad is_adi FROM hakedisler h JOIN sozlesmeler s ON s.id=h.sozlesme_id LEFT JOIN is_tanimlari i ON i.id=h.is_id",
        ];
        ...
        $sortColumns = [
            ...
            'hakedisler'=>['id','yil','ay','sozlesme_adi','is_adi','surum','toplam_tutar','durum'],
        ];
        $requestedSort=(string)($filters['sort']??'');$sort=in_array($requestedSort,$sortColumns[$entity],true)?$requestedSort:'id';$direction=strtoupper((string)($filters['dir']??'DESC'))==='ASC'?'ASC':'DESC';
        if($requestedSort===''&&$entity==='bordrolar')$sql.=" ORDER BY q.is_adi ASC,q.yil DESC,q.ay DESC,q.surum DESC LIMIT {$size} OFFSET {$offset}";
        elseif($requestedSort===''&&$entity==='hakedisler')$sql.=" ORDER BY q.sozlesme_adi ASC,q.yil DESC,q.ay DESC,q.is_adi ASC LIMIT {$size} OFFSET {$offset}";
        else$sql.=" ORDER BY q.{$sort} {$direction} LIMIT {$size} OFFSET {$offset}";
    ...
    public function benefitAnalysis(array $filters=[]):array
    {
        $periods=$this->db->query("SELECT DISTINCT yil,ay FROM bordrolar WHERE aktif=1 AND durum='tamamlandi' ORDER BY yil DESC,ay DESC")->fetchAll();
        if(!$periods)return['periods'=>[],'year'=>(int)date('Y'),'month'=>(int)date('n'),'rights'=>[],'people'=>[],'rows'=>[],'metrics'=>['jobs'=>0,'personnel'=>0,'received'=>0,'not_received'=>0,'rights'=>0,'amount'=>0]];
        $year=(int)($filters['yil']??0);$month=(int)($filters['ay']??0);
        $available=array_filter($periods,static fn(array$p):bool=>$year<1||(int)$p['yil']===$year);
        $selected=$available?reset($available):$periods[0];
        if($year<1)$year=(int)$selected['yil'];
        if($month<1)$month=(int)$selected['ay'];
        $exists=false;foreach($periods as$period)if((int)$period['yil']===$year&&(int)$period['ay']===$month){$exists=true;break;}
        if(!$exists){$year=(int)$selected['yil'];$month=(int)$selected['ay'];}
        $requestedView=(string)($filters['view']??'job');$view=in_array($requestedView,['job','person','audit'],true)?$requestedView:'job';
        $jobId=max(0,(int)($filters['is_id']??0));$personId=max(0,(int)($filters['personel_id']??0));$rightId=max(0,(int)($filters['right_id']??0));
        $keyword=in_array((string)($filters['keyword']??''),['yemek','yol'],true)?(string)$filters['keyword']:'';
        $requestedStatus=(string)($filters['status']??'all');$status=in_array($requestedStatus,['all','received','not_received','incorrect'],true)?$requestedStatus:'all';
        $baseWhere="b.aktif=1 AND b.durum='tamamlandi' AND b.yil=:yil AND b.ay=:ay";$baseArgs=['yil'=>$year,'ay'=>$month];
        if($jobId>0){$baseWhere.=' AND b.is_id=:is_id';$baseArgs['is_id']=$jobId;}
        if($personId>0){$baseWhere.=' AND bp.personel_id=:personel_id';$baseArgs['personel_id']=$personId;}
        $definitions=$this->db->query("SELECT h.id,h.hak_grup_kodu,h.hak_adi,h.bordro_kalem_adi,h.birim,h.birim_fiyat,h.hesaplama_sekli,t.ad donem_adi FROM sendikal_haklar h JOIN tis_donemleri t ON t.id=h.tis_donem_id WHERE h.aktif=1 AND NOT EXISTS(SELECT 1 FROM sendikal_haklar n WHERE n.onceki_hak_id=h.id) ORDER BY h.hak_adi,t.baslangic_tarihi DESC")->fetchAll();
        $rightType=$keyword;if($rightType===''&&$rightId>0)foreach($definitions as$definition)if((int)$definition['id']===$rightId){$normalized=mb_strtoupper((string)$definition['hak_adi'].' '.(string)$definition['bordro_kalem_adi'],'UTF-8');if(str_contains($normalized,'YEMEK'))$rightType='yemek';elseif(str_contains($normalized,'YOL'))$rightType='yol';break;}
        $expectedColumn=$rightType==='yemek'?'yemek_gunu':($rightType==='yol'?'yol_gunu':'');
        $rightJoin="h.tis_donem_id=b.tis_donem_id AND h.aktif=1 AND h.gecerlilik_baslangic<=LAST_DAY(STR_TO_DATE(CONCAT(b.yil,'-',LPAD(b.ay,2,'0'),'-01'),'%Y-%m-%d')) AND (h.gecerlilik_bitis IS NULL OR h.gecerlilik_bitis>=STR_TO_DATE(CONCAT(b.yil,'-',LPAD(b.ay,2,'0'),'-01'),'%Y-%m-%d')) AND (UPPER(TRIM(h.bordro_kalem_adi))=UPPER(TRIM(k.kalem_adi)) OR UPPER(TRIM(h.hak_adi))=UPPER(TRIM(k.kalem_adi)))";
        if($rightId>0)$rightJoin.=" AND COALESCE(h.hak_grup_kodu,CONCAT('ID-',h.id))=COALESCE((SELECT selected_right.hak_grup_kodu FROM sendikal_haklar selected_right WHERE selected_right.id=".(int)$rightId."),CONCAT('ID-',".(int)$rightId."))";
        elseif($keyword!==''){$needle=$keyword==='yemek'?'YEMEK':'YOL';$rightJoin.=" AND (UPPER(h.hak_adi) LIKE '%{$needle}%' OR UPPER(h.bordro_kalem_adi) LIKE '%{$needle}%')";}
        $rightsStmt=$this->db->prepare("SELECT DISTINCT h.id,h.hak_adi FROM bordrolar b JOIN bordro_personelleri bp ON bp.bordro_id=b.id JOIN bordro_kalemleri k ON k.bordro_personel_id=bp.id JOIN sendikal_haklar h ON {$rightJoin} WHERE b.aktif=1 AND b.durum='tamamlandi' AND b.yil=? AND b.ay=? ORDER BY h.hak_adi");$rightsStmt->execute([$year,$month]);$rights=$rightsStmt->fetchAll();
        $peopleWhere="b.aktif=1 AND b.durum='tamamlandi' AND b.yil=:yil AND b.ay=:ay";$peopleArgs=['yil'=>$year,'ay'=>$month];if($jobId>0){$peopleWhere.=' AND b.is_id=:is_id';$peopleArgs['is_id']=$jobId;}
        $peopleStmt=$this->db->prepare("SELECT DISTINCT p.id,p.ad_soyad,COALESCE(NULLIF(pr.personel_kodu,''),NULLIF(p.sgk_sicil_no,''),CONCAT('P-',LPAD(p.id,5,'0'))) personel_kodu FROM bordrolar b JOIN bordro_personelleri bp ON bp.bordro_id=b.id AND bp.personel_id IS NOT NULL JOIN personeller p ON p.id=bp.personel_id LEFT JOIN ik_personel_profilleri pr ON pr.personel_id=p.id WHERE {$peopleWhere} ORDER BY p.ad_soyad");$peopleStmt->execute($peopleArgs);$people=$peopleStmt->fetchAll();
        $metricsSql="SELECT COUNT(DISTINCT b.is_id) jobs,COUNT(DISTINCT bp.personel_id) personnel,COUNT(DISTINCT CASE WHEN h.id IS NOT NULL AND (ABS(k.miktar)>.004 OR ABS(k.tutar)>.004) THEN bp.personel_id END) received,COUNT(DISTINCT h.id) rights,COALESCE(SUM(CASE WHEN h.id IS NOT NULL THEN k.tutar ELSE 0 END),0) amount FROM bordrolar b JOIN bordro_personelleri bp ON bp.bordro_id=b.id AND bp.personel_id IS NOT NULL LEFT JOIN bordro_kalemleri k ON k.bordro_personel_id=bp.id LEFT JOIN sendikal_haklar h ON {$rightJoin} WHERE {$baseWhere}";$metricsStmt=$this->db->prepare($metricsSql);$metricsStmt->execute($baseArgs);$metrics=$metricsStmt->fetch()?:[];$metrics['jobs']=(int)($metrics['jobs']??0);$metrics['personnel']=(int)($metrics['personnel']??0);$metrics['received']=(int)($metrics['received']??0);$metrics['not_received']=max(0,$metrics['personnel']-$metrics['received']);$metrics['rights']=(int)($metrics['rights']??0);$metrics['amount']=(float)($metrics['amount']??0);$metrics['incorrect']=0;
        $auditRow=static function(array$row)use($expectedColumn):array{$row['quantity']=(float)($row['quantity']??0);$row['amount']=(float)($row['amount']??0);$row['received']=abs($row['quantity'])>.004||abs($row['amount'])>.004;$row['expected_quantity']=$expectedColumn!==''?(float)($row['expected_quantity']??0):null;$row['difference']=$row['expected_quantity']===null?null:round($row['quantity']-$row['expected_quantity'],2);if($row['expected_quantity']===null){$row['audit_status']=$row['received']?'received':'not_received';$row['audit_label']=$row['received']?'Aldı':'Almadı';}elseif($row['expected_quantity']>.004&&!$row['received']){$row['audit_status']='missing';$row['audit_label']='Alması gerekirken almadı';}elseif($row['expected_quantity']<=.004&&$row['received']){$row['audit_status']='unexpected';$row['audit_label']='Hak etmediği halde aldı';}elseif(abs((float)$row['difference'])>.004){$row['audit_status']='mismatch';$row['audit_label']=$row['difference']>0?'Fazla miktar':'Eksik miktar';}else{$row['audit_status']='correct';$row['audit_label']='Doğru';}$row['incorrect']=in_array($row['audit_status'],['missing','unexpected','mismatch'],true);return$row;};
        $expectedSelect=$expectedColumn!==''?"MAX(COALESCE(pu.{$expectedColumn},0))":'NULL';
        $personSql="SELECT p.id personel_id,COALESCE(NULLIF(pr.personel_kodu,''),NULLIF(p.sgk_sicil_no,''),CONCAT('P-',LPAD(p.id,5,'0'))) personel_kodu,p.ad_soyad,b.is_id,i.ad is_adi,GROUP_CONCAT(DISTINCT CASE WHEN h.id IS NOT NULL AND (ABS(k.miktar)>.004 OR ABS(k.tutar)>.004) THEN h.hak_adi END ORDER BY h.hak_adi SEPARATOR ', ') haklar,ROUND(SUM(CASE WHEN h.id IS NOT NULL THEN k.miktar ELSE 0 END),2) quantity,ROUND(SUM(CASE WHEN h.id IS NOT NULL THEN k.tutar ELSE 0 END),2) amount,{$expectedSelect} expected_quantity FROM bordrolar b JOIN is_tanimlari i ON i.id=b.is_id JOIN bordro_personelleri bp ON bp.bordro_id=b.id AND bp.personel_id IS NOT NULL JOIN personeller p ON p.id=bp.personel_id LEFT JOIN ik_personel_profilleri pr ON pr.personel_id=p.id LEFT JOIN puantajlar pu ON pu.bordro_id=b.id AND pu.personel_id=p.id LEFT JOIN bordro_kalemleri k ON k.bordro_personel_id=bp.id LEFT JOIN sendikal_haklar h ON {$rightJoin} WHERE {$baseWhere} GROUP BY p.id,pr.personel_kodu,p.sgk_sicil_no,p.ad_soyad,b.is_id,i.ad ORDER BY p.ad_soyad";
        $personStmt=$this->db->prepare($personSql);$personStmt->execute($baseArgs);$allPersonRows=[];foreach($personStmt->fetchAll()as$row)$allPersonRows[]=$auditRow($row);
        if($expectedColumn!=='')$metrics['incorrect']=count(array_filter($allPersonRows,static fn(array$row):bool=>(bool)$row['incorrect']));
        if($view==='job'){
            $countsStmt=$this->db->prepare("SELECT b.is_id,i.ad is_adi,COUNT(DISTINCT bp.personel_id) personnel FROM bordrolar b JOIN is_tanimlari i ON i.id=b.is_id JOIN bordro_personelleri bp ON bp.bordro_id=b.id AND bp.personel_id IS NOT NULL WHERE {$baseWhere} GROUP BY b.is_id,i.ad ORDER BY i.ad");$countsStmt->execute($baseArgs);$counts=[];foreach($countsStmt->fetchAll()as$count)$counts[(int)$count['is_id']]=$count;
            $actualSql="SELECT b.is_id,i.ad is_adi,h.id right_id,h.hak_adi,COUNT(DISTINCT CASE WHEN ABS(k.miktar)>.004 OR ABS(k.tutar)>.004 THEN bp.personel_id END) received,ROUND(SUM(k.miktar),2) quantity,ROUND(SUM(k.tutar),2) amount FROM bordrolar b JOIN is_tanimlari i ON i.id=b.is_id JOIN bordro_personelleri bp ON bp.bordro_id=b.id AND bp.personel_id IS NOT NULL JOIN bordro_kalemleri k ON k.bordro_personel_id=bp.id JOIN sendikal_haklar h ON {$rightJoin} WHERE {$baseWhere} GROUP BY b.is_id,i.ad,h.id,h.hak_adi ORDER BY i.ad,h.hak_adi";$actualStmt=$this->db->prepare($actualSql);$actualStmt->execute($baseArgs);$rows=[];foreach($actualStmt->fetchAll()as$row){$total=(int)($counts[(int)$row['is_id']]['personnel']??0);$row['personnel']=$total;$row['received']=(int)$row['received'];$row['not_received']=max(0,$total-$row['received']);$row['quantity']=(float)$row['quantity'];$row['amount']=(float)$row['amount'];if($status==='received'&&$row['received']<1)continue;if($status==='not_received'&&$row['not_received']<1)continue;if($status==='incorrect')continue;$rows[]=$row;}
            if($rightId>0){$rightName='Seçilen hak';foreach($definitions as$right)if((int)$right['id']===$rightId){$rightName=$right['hak_adi'];break;}foreach($counts as$isId=>$count){$found=false;foreach($rows as$row)if((int)$row['is_id']===$isId){$found=true;break;}if(!$found&&!in_array($status,['received','incorrect'],true))$rows[]=['is_id'=>$isId,'is_adi'=>$count['is_adi'],'right_id'=>$rightId,'hak_adi'=>$rightName,'personnel'=>(int)$count['personnel'],'received'=>0,'not_received'=>(int)$count['personnel'],'quantity'=>0.0,'amount'=>0.0];}}
        }else{
            $rows=[];if(!($view==='audit'&&$rightId<1&&$keyword===''))foreach($allPersonRows as$row){if($status==='received'&&!$row['received'])continue;if($status==='not_received'&&$row['received'])continue;if($status==='incorrect'&&!$row['incorrect'])continue;$rows[]=$row;}
        }
        $monthlyRows=[];$availableMonths=[];
        if($view==='audit'){$distribution=$this->benefitMovementDistribution($year,$month,$jobId,$personId,$rightId,$keyword,$status,$definitions);$monthlyRows=$distribution['rows'];$availableMonths=$distribution['months'];}
        $timeline=[];$selectedPerson=null;
        if($personId>0){
            foreach($people as$person)if((int)$person['id']===$personId){$selectedPerson=$person;break;}
            if(!$selectedPerson){$selectedStmt=$this->db->prepare("SELECT p.id,p.ad_soyad,COALESCE(NULLIF(pr.personel_kodu,''),NULLIF(p.sgk_sicil_no,''),CONCAT('P-',LPAD(p.id,5,'0'))) personel_kodu FROM personeller p LEFT JOIN ik_personel_profilleri pr ON pr.personel_id=p.id WHERE p.id=?");$selectedStmt->execute([$personId]);$selectedPerson=$selectedStmt->fetch()?:null;}
            $timelineWhere="b.aktif=1 AND b.durum='tamamlandi' AND b.yil=:timeline_year AND bp.personel_id=:timeline_person";$timelineArgs=['timeline_year'=>$year,'timeline_person'=>$personId];if($jobId>0){$timelineWhere.=' AND b.is_id=:timeline_job';$timelineArgs['timeline_job']=$jobId;}
            $timelineSql="SELECT b.ay,GROUP_CONCAT(DISTINCT i.ad ORDER BY i.ad SEPARATOR ', ') is_adi,ROUND(SUM(CASE WHEN h.id IS NOT NULL THEN k.miktar ELSE 0 END),2) quantity,ROUND(SUM(CASE WHEN h.id IS NOT NULL THEN k.tutar ELSE 0 END),2) amount,{$expectedSelect} expected_quantity FROM bordrolar b JOIN is_tanimlari i ON i.id=b.is_id JOIN bordro_personelleri bp ON bp.bordro_id=b.id AND bp.personel_id IS NOT NULL LEFT JOIN puantajlar pu ON pu.bordro_id=b.id AND pu.personel_id=bp.personel_id LEFT JOIN bordro_kalemleri k ON k.bordro_personel_id=bp.id LEFT JOIN sendikal_haklar h ON {$rightJoin} WHERE {$timelineWhere} GROUP BY b.ay ORDER BY b.ay";$timelineStmt=$this->db->prepare($timelineSql);$timelineStmt->execute($timelineArgs);$timelineByMonth=[];foreach($timelineStmt->fetchAll()as$row){$row=$auditRow($row);$row['has_data']=true;$timelineByMonth[(int)$row['ay']]=$row;}for($calendarMonth=1;$calendarMonth<=12;$calendarMonth++)$timeline[]=$timelineByMonth[$calendarMonth]??['ay'=>$calendarMonth,'is_adi'=>'','quantity'=>0.0,'amount'=>0.0,'expected_quantity'=>null,'difference'=>null,'received'=>false,'audit_status'=>'no_data','audit_label'=>'Bordro yok','incorrect'=>false,'has_data'=>false];
        }
        return['periods'=>$periods,'year'=>$year,'month'=>$month,'view'=>$view,'rights'=>$rights,'definitions'=>$definitions,'people'=>$people,'rows'=>$rows,'monthly_rows'=>$monthlyRows,'available_months'=>$availableMonths,'metrics'=>$metrics,'right_type'=>$rightType,'audit_requires_right'=>false,'selected_person'=>$selectedPerson,'timeline'=>$timeline];
    }

    private function benefitMovementDistribution(int$year,int$selectedMonth,int$jobId,int$personId,int$rightId,string$keyword,string$status,array$definitions):array
    {
        $normalize=static function($value):string{$text=mb_strtolower(trim((string)$value),'UTF-8');$text=str_replace("\u{0307}",'',$text);$text=strtr($text,['ı'=>'i','ş'=>'s','ğ'=>'g','ü'=>'u','ö'=>'o','ç'=>'c','.'=>'','_'=>' ','('=>' ',')'=>' ']);return(string)preg_replace('/\s+/u',' ',$text);};
        $selectedDefinition=null;if($rightId>0)foreach($definitions as$definition)if((int)$definition['id']===$rightId){$selectedDefinition=$definition;break;}
        $targetLabel='';$targetAliases=[];
        if($selectedDefinition){$targetLabel=(string)$selectedDefinition['hak_adi'];$targetAliases=array_unique(array_filter([$normalize($selectedDefinition['hak_adi']),$normalize($selectedDefinition['bordro_kalem_adi']??'')]));}
        elseif($keyword!==''){$needle=$normalize($keyword);foreach($definitions as$definition)if(str_contains($normalize(($definition['hak_adi']??'').' '.($definition['bordro_kalem_adi']??'')),$needle)){$targetLabel=(string)$definition['hak_adi'];break;}if($targetLabel==='')$targetLabel=$keyword==='yemek'?'YEMEK YARDIMI':'YOL YARDIMI';}
        $payrollSql="SELECT b.id,b.ay,b.is_id,i.ad is_adi FROM bordrolar b JOIN is_tanimlari i ON i.id=b.is_id WHERE b.aktif=1 AND b.durum='tamamlandi' AND b.yil=:yil";$payrollArgs=['yil'=>$year];if($jobId>0){$payrollSql.=' AND b.is_id=:is_id';$payrollArgs['is_id']=$jobId;}$payrollSql.=' ORDER BY i.ad,b.ay,b.id';$payrollStmt=$this->db->prepare($payrollSql);$payrollStmt->execute($payrollArgs);$payrolls=$payrollStmt->fetchAll();
        $codeRows=$this->db->query("SELECT p.id,COALESCE(NULLIF(pr.personel_kodu,''),NULLIF(p.sgk_sicil_no,''),CONCAT('P-',LPAD(p.id,5,'0'))) personel_kodu FROM personeller p LEFT JOIN ik_personel_profilleri pr ON pr.personel_id=p.id")->fetchAll();$codes=[];foreach($codeRows as$code)$codes[(int)$code['id']]=$code['personel_kodu'];
        $people=[];$matrix=[];$monthSet=[];
        foreach($payrolls as$payroll){$month=(int)$payroll['ay'];$monthSet[$month]=true;$movement=$this->payrollMovements((int)$payroll['id']);$rightMap=[];foreach($movement['rights']as$right)$rightMap[(string)$right['key']]=$right;
            foreach($movement['rows']as$person){$personKey=$person['personel_id']?'P'.(int)$person['personel_id']:'N'.$normalize($person['ad_soyad']);if($personId>0&&(int)($person['personel_id']??0)!==$personId)continue;$scopeKey=(int)$payroll['is_id'].':'.$personKey;if(!isset($people[$scopeKey]))$people[$scopeKey]=['personel_id'=>$person['personel_id']?(int)$person['personel_id']:null,'personel_kodu'=>$person['personel_id']?($codes[(int)$person['personel_id']]??('P-'.str_pad((string)$person['personel_id'],5,'0',STR_PAD_LEFT))):'—','ad_soyad'=>$person['ad_soyad'],'is_id'=>(int)$payroll['is_id'],'is_adi'=>$payroll['is_adi'],'months'=>[]];$people[$scopeKey]['months'][$month]=true;
                foreach($rightMap as$key=>$right){$amount=(float)($person['haklar'][$key]??0);$quantity=(float)($person['hak_miktarlari'][$key]??0);if(abs($amount)<.005&&abs($quantity)<.005)continue;$rawLabel=(string)$right['label'];$normalizedLabel=$normalize($rawLabel);if($selectedDefinition&&!in_array($normalizedLabel,$targetAliases,true))continue;if($keyword!==''&&!str_contains($normalizedLabel,$normalize($keyword)))continue;$label=$targetLabel!==''?$targetLabel:$rawLabel;$rowKey=$scopeKey.':'.$normalize($label);if(!isset($matrix[$rowKey]))$matrix[$rowKey]=['personel_id'=>$people[$scopeKey]['personel_id'],'personel_kodu'=>$people[$scopeKey]['personel_kodu'],'ad_soyad'=>$people[$scopeKey]['ad_soyad'],'is_id'=>$people[$scopeKey]['is_id'],'is_adi'=>$people[$scopeKey]['is_adi'],'hak_adi'=>$label,'months'=>[],'total_amount'=>0.0,'total_quantity'=>0.0,'_person_key'=>$scopeKey];if(!isset($matrix[$rowKey]['months'][$month]))$matrix[$rowKey]['months'][$month]=['ay'=>$month,'quantity'=>0.0,'amount'=>0.0,'received'=>true,'audit_status'=>'received','audit_label'=>'Aldı','incorrect'=>false,'has_data'=>true];$matrix[$rowKey]['months'][$month]['amount']+=$amount;$matrix[$rowKey]['months'][$month]['quantity']+=$quantity;$matrix[$rowKey]['total_amount']+=$amount;$matrix[$rowKey]['total_quantity']+=$quantity;}
            }
        }
        if($targetLabel!=='')foreach($people as$scopeKey=>$person){$rowKey=$scopeKey.':'.$normalize($targetLabel);if(!isset($matrix[$rowKey]))$matrix[$rowKey]=['personel_id'=>$person['personel_id'],'personel_kodu'=>$person['personel_kodu'],'ad_soyad'=>$person['ad_soyad'],'is_id'=>$person['is_id'],'is_adi'=>$person['is_adi'],'hak_adi'=>$targetLabel,'months'=>[],'total_amount'=>0.0,'total_quantity'=>0.0,'_person_key'=>$scopeKey];}
        ksort($monthSet);$months=array_map('intval',array_keys($monthSet));$rows=[];foreach($matrix as$row){$personMonths=$people[$row['_person_key']]['months']??[];unset($row['_person_key']);foreach($months as$month){if(isset($row['months'][$month])){$row['months'][$month]['amount']=round((float)$row['months'][$month]['amount'],2);$row['months'][$month]['quantity']=round((float)$row['months'][$month]['quantity'],2);continue;}$row['months'][$month]=isset($personMonths[$month])?['ay'=>$month,'quantity'=>0.0,'amount'=>0.0,'received'=>false,'audit_status'=>'not_received','audit_label'=>'Almadı','incorrect'=>false,'has_data'=>true]:['ay'=>$month,'quantity'=>0.0,'amount'=>0.0,'received'=>false,'audit_status'=>'no_data','audit_label'=>'Bordro yok','incorrect'=>false,'has_data'=>false];}ksort($row['months']);$selected=$row['months'][$selectedMonth]??null;if($status==='received'&&(!$selected||!$selected['received']))continue;if($status==='not_received'&&(!$selected||!$selected['has_data']||$selected['received']))continue;if($status==='incorrect')continue;$row['total_amount']=round((float)$row['total_amount'],2);$row['total_quantity']=round((float)$row['total_quantity'],2);$rows[]=$row;}
        usort($rows,static fn(array$a,array$b):int=>[$a['is_adi'],$a['ad_soyad'],$a['hak_adi']]<=>[$b['is_adi'],$b['ad_soyad'],$b['hak_adi']]);return['rows'=>$rows,'months'=>$months];
    }
    ...
    public function generateProgressFromPayroll(int$id):int
    {
        ...
        $rights=[];foreach($movement['rights']as$right){if((int)($right['id']??0)<1){$findBalance=$this->db->prepare("SELECT id,hak_adi,birim,birim_fiyat FROM sendikal_haklar WHERE tis_donem_id=? AND hak_adi='DİĞER KAZANÇ (BORDRO FARKI)' AND aktif=1 ORDER BY id DESC LIMIT 1");$findBalance->execute([(int)$payroll['tis_donem_id']]);$stored=$findBalance->fetch();if(!$stored){$this->db->prepare("INSERT INTO sendikal_haklar(tis_donem_id,hak_grup_kodu,surum,hak_adi,bordro_kalem_adi,birim,birim_fiyat,hesaplama_sekli,hakedise_dahil,gecerlilik_baslangic,aktif) VALUES(?,UUID(),1,'DİĞER KAZANÇ (BORDRO FARKI)','DİĞER KAZANÇ','tutar',0,'bordro_tutari',1,?,1)")->execute([(int)$payroll['tis_donem_id'],sprintf('%04d-%02d-01',(int)$payroll['yil'],(int)$payroll['ay'])]);$stored=['id'=>(int)$this->db->lastInsertId(),'hak_adi'=>'DİĞER KAZANÇ (BORDRO FARKI)','birim'=>'tutar','birim_fiyat'=>0.0];}$right=array_merge($right,['id'=>(int)$stored['id'],'label'=>$stored['hak_adi'],'birim'=>$stored['birim'],'birim_fiyat'=>(float)$stored['birim_fiyat']]);}$rights[$right['key']]=$right;}$year=(int)$payroll['yil'];$month=(int)$payroll['ay'];$contractId=(int)$payroll['sozlesme_id'];$cost=$movement['cost_summary'];if(empty($cost['bordro_ozeti_var']))throw new RuntimeException('İşveren primleri bordro özetinden alınamadı. Bordroyu yeniden yükleyin.');$service=round((float)$cost['baz_toplam'],2);$current=round((float)$cost['toplam_hakedis_tutari'],2);
        $this->db->beginTransaction();
        try{
            $jobId=(int)$payroll['is_id'];$q=$this->db->prepare('SELECT COALESCE(MAX(surum),0)+1 FROM hakedisler WHERE yil=? AND ay=? AND sozlesme_id=?');$q->execute([$year,$month,$contractId]);$version=(int)$q->fetchColumn();$this->db->prepare('UPDATE hakedisler SET aktif=0 WHERE yil=? AND ay=? AND sozlesme_id=? AND is_id=? AND aktif=1')->execute([$year,$month,$contractId,$jobId]);
            $this->db->prepare('INSERT INTO hakedisler(yil,ay,sozlesme_id,is_id,surum,toplam_tutar,olusturan_kullanici_id) VALUES(?,?,?,?,?,?,?)')->execute([$year,$month,$contractId,$jobId,$version,$current,$this->userId]);$progressId=(int)$this->db->lastInsertId();$this->db->prepare('INSERT INTO hakedis_bordrolari(hakedis_id,bordro_id) VALUES(?,?)')->execute([$progressId,$id]);
    ...
    {
        $old=$this->progressEditData($id);$this->db->beginTransaction();try{$this->db->prepare('DELETE FROM hakedis_genel_kalemleri WHERE hakedis_id=?')->execute([$id]);$this->db->prepare('DELETE FROM hakedis_mali_ozetleri WHERE hakedis_id=?')->execute([$id]);$this->db->prepare('DELETE FROM hakedis_detaylari WHERE hakedis_id=?')->execute([$id]);$this->db->prepare('DELETE FROM hakedis_bordrolari WHERE hakedis_id=?')->execute([$id]);$this->db->prepare('DELETE FROM hakedisler WHERE id=?')->execute([$id]);if((int)$old['aktif']===1){$q=$this->db->prepare('SELECT id FROM hakedisler WHERE yil=? AND ay=? AND sozlesme_id=? AND (is_id<=>?) ORDER BY surum DESC LIMIT 1');$q->execute([(int)$old['yil'],(int)$old['ay'],(int)$old['sozlesme_id'],$old['is_id']!==null?(int)$old['is_id']:null]);$previousId=(int)$q->fetchColumn();if($previousId)$this->db->prepare('UPDATE hakedisler SET aktif=1 WHERE id=?')->execute([$previousId]);}$this->log('sil','hakedisler',$id,null,json_encode($old,JSON_UNESCAPED_UNICODE),null);$this->db->commit();}catch(Throwable$e){$this->db->rollBack();throw$e;}
    }
